...
//
// The size of the data structure used by arrayBasedEscaper is proportional to the highest valued byte that has a replacement.
...
func NewEscaperMap(replMap map[byte]string) *EscaperMap {
	return &EscaperMap{createReplArray(replMap)}
}

// GetReplacements returns the non-nil array of replacements for fast lookup.
func (em *EscaperMap) GetReplacements() [][]byte {
	return em.replArray
}

// createReplArray creates a replacement array from the given map.
// The returned array is a linear lookup table of replacement sequences indexed by the original byte value.
func createReplArray(m map[byte]string) [][]byte {
	if len(m) == 0 {
		return [][]byte{}
	}
